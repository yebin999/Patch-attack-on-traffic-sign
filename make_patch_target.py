"""
@author:yebin
@file:make_patch_target.py
@IDE:Pycharm
@time:2020/12/7 下午9:25
@function:生成patch，有目标攻击
@example:
@tip:
"""
import argparse
...
def save_patch(patch, save_path):
    ...
    im.save(os.path.join(save_path, "patch.png"))
...
def train(epoch, patch, patch_shape):
    ...
    for batch_indx, (images, labels) in enumerate(cifar100_training_loader):
        ...
        patch = new_patch
        np.save(os.path.join(args.patch_path, "patch.npy"), patch)
...
def test(epoch, patch, patch_shape):
    ...
    for batch_indx, (images, labels) in enumerate(cifar100_test_loader):
        ...
        if prediction.data.max(1)[1] != labels.data[0] and (labels.data[0] == target):  # TODO 考虑batchsize大于1的情况，暂时只做batchsize为1的情况
