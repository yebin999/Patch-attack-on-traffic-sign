"""
@author:yebin
@file:make_patch_ensemble.py
@time:2020/12/24 下午2:57
@function:white box ensemble training
@example:
@tip:
"""
import argparse
...
def attack(x, patch, mask, epoch, indx):
    ...
    x_out1 = F.softmax(net1(x), dim=1)      #前向推理的结果 (1, 43)
    x_out2 = F.softmax(net2(x), dim=1)
    x_out3 = F.softmax(net3(x), dim=1)
    # x_ = x_out.data[:][target]
    target_prob1 = x_out1.data[0][target]
    target_prob2 = x_out2.data[0][target]
    target_prob3 = x_out3.data[0][target]

    adv_x = torch.mul((1 - mask), x) + torch.mul(mask, patch)

    count = 0
    lr = args.lr
    while (conf_target > target_prob1 or conf_target > target_prob2 or conf_target > target_prob3) and count < args.max_count:
        count += 1
        adv_x = Variable(adv_x.data, requires_grad=True)
        adv_out1 = F.log_softmax(net1(adv_x), dim=1)     # (1, 43)
        adv_out2 = F.log_softmax(net2(adv_x), dim=1)
        adv_out3 = F.log_softmax(net3(adv_x), dim=1)

        adv_out1_probs, adv_out1_labels = adv_out1.max(1)
        adv_out2_probs, adv_out2_labels = adv_out2.max(1)
        adv_out3_probs, adv_out3_labels = adv_out3.max(1)

        Loss = -(adv_out1[0][target] + adv_out2[0][target] + adv_out3[0][target]) / 3
        ...
        out1 = F.softmax(net1(adv_x), dim=1)
        out2 = F.softmax(net2(adv_x), dim=1)
        out3 = F.softmax(net3(adv_x), dim=1)
        target_prob1 = out1.data[0][target]
        target_prob2 = out2.data[0][target]
        target_prob3 = out3.data[0][target]
        # y_argmax_prob = out.data.max(1)[0][0]

        print("%s_%d_[%d/840]count%d_target_prob1:%f, target_prob2:%f, target_prob3:%f" %("epoch", epoch, indx, count, target_prob1, target_prob2, target_prob3))
...
def train(epoch, patch, patch_shape):
    ...
    for batch_indx, (images, labels) in enumerate(cifar100_training_loader):
        ...
        prediction1 = net1(images)
        prediction2 = net2(images)
        prediction3 = net3(images)
        

        if prediction1.data.max(1)[1][0] != labels.data[0] or prediction2.data.max(1)[1][0] != labels.data[0] or prediction3.data.max(1)[1][0] != labels.data[0] or labels.data[0] == target:  # TODO 考虑batchsize大于1的情况，暂时只做batchsize为1的情况
            continue
        ...
        adv_label1 = net1(adv_x).data.max(1)[1][0]
        adv_label2 = net2(adv_x).data.max(1)[1][0]
        adv_label3 = net3(adv_x).data.max(1)[1][0]
        ori_label = labels[0]
    ...
    return patch, success / total, total
...
args.image_size = 256
args.net1 = 'vgg16'
args.weights1 = '/data/user1/yebin/work/ModelClasstest/pytorch-cifar100/checkpoint/vgg16/GTSRB256to256/vgg16-191-best.pth'
args.net2 = 'googlenet'
args.weights2 = '/data/user1/yebin/work/ModelClasstest/pytorch-cifar100/checkpoint/googlenet/Tuesday_29_December_2020_17h_57m_59s/googlenet-128-best.pth'
args.net3 = 'resnet34'
args.weights3 = '/data/user1/yebin/work/ModelClasstest/pytorch-cifar100/checkpoint/resnet34/Saturday_26_December_2020_04h_00m_39s/resnet34-121-best.pth'
...
args.lr = 1
args.loss = 1
args.patch_path = "/data/user1/yebin/work/ModelClasstest/pytorch-cifar100/train_res/img256/ensembleAttack/mul_ensemble_5%"
...
print('===> 开始生成分类器，加载模型中 <===')
net1 = get_network(args.net1, args.gpu)
net1.load_state_dict(torch.load(args.weights1))   #TODO:固定网络参数梯度，防止在训练中更新（目前尚未固定梯度）
net2 = get_network(args.net2, args.gpu)
net2.load_state_dict(torch.load(args.weights2))
net3 = get_network(args.net3, args.gpu)
net3.load_state_dict(torch.load(args.weights3))
...
if __name__ == '__main__':
    ...
    with open(args.log_dir, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "clean_right_num", "train1_success", "train2_success", "train3_success", "train_ensemble_success", "test1_success", "test2_success", "test3_success", "test_ensemble_success", "train_process_success_rate"])


    best_patch_epoch, best_patch_success_rate = 0, 0
    mean, std = settings.CIFAR100_TRAIN_MEAN, settings.CIFAR100_TRAIN_STD
    for epoch in range(1, args.epochs + 1):
        patch, train_process_success_rate, clean_right = train(epoch, patch, patch_shape)
        # np.save(os.path.join(args.patch_path, "%s_%d_patch.npy" % ("epoch", epoch)), patch)
        ...
        train_success_rate = test_ensemble_patch(patch_type, target, patch, cifar100_training_loader, net1, net2, net3, gpu=args.gpu, patch_shape=patch_shape, image_size=args.image_size)
        # print("train_success_rate:", train_success_rate[0], train_success_rate[1], train_success_rate[2], train_success_rate[3])
        print("Epoch:{} Patch attack success rate on trainset: {:.3f}%, {:.3f}%, {:.3f}%, {:.3f}%".format(epoch, 100 * train_success_rate[0], 100 * train_success_rate[1], 100 * train_success_rate[2], 100 * train_success_rate[3]))
        test_success_rate = test_ensemble_patch(patch_type, target, patch, cifar100_test_loader, net1, net2, net3, gpu=args.gpu, patch_shape=patch_shape, image_size=args.image_size)
        print("Epoch:{} Patch attack success rate on testset: {:.3f}%, {:.3f}%, {:.3f}%, {:.3f}%".format(epoch, 100 * test_success_rate[0], 100 * test_success_rate[1], 100 * test_success_rate[2], 100 * train_success_rate[3]))
        ...
        with open(args.log_dir, 'a') as f:
            writer = csv.writer(f)
            writer.writerow([epoch, clean_right, train_success_rate[0], train_success_rate[1], train_success_rate[2], train_success_rate[3], test_success_rate[0], test_success_rate[1], test_success_rate[2], test_success_rate[3], train_process_success_rate])

        if test_success_rate[3] > best_patch_success_rate:
            best_patch_success_rate = test_success_rate[2]
