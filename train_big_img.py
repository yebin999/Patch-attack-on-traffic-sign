"""
@author:yebin
@file:train_big_img.py
@IDE:Pycharm
@time:2020/12/18 下午2:03
@function:训练大尺寸的图片256*256,使用之前注意修改网络结构，原网络结构是训练32*32图像的
@example:
@tip:
"""
import os
...
def train(epoch):
    ...
    start = time.time()
    if args.pretrain:
        check = torch.load(args.pretrain)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-pretrain', type=str, default='', help='Loading pre-train')
    ...
    args.lr = 0.01
    args.warm = 1
    args.train_path = "/data/user1/yebin/work/Data_lib/GTSRB_原数据/train"
    args.test_path = "/data/user1/yebin/work/Data_lib/GTSRB_原数据/test"
    args.resume = False
    args.pretrain = None
